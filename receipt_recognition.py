...
- Сума (total): спочатку завжди знайди суму на чеку/рахунку, потім порівняй із сумою або формулою в підписі. Не бери суму з підпису автоматично. Використовуй підпис тільки якщо різниця пояснена або логічна (додаткова доставка, експлуатація авто, комісія, кілька чеків, знижка, попередня оплата, формула тощо). Якщо підпис містить відсоток додаткових витрат ("30% від суми заправки", "експлуатація авто 30%") — додай цей відсоток до суми чека: total = сума чека + сума чека * відсоток / 100. Якщо підпис просто суперечить чеку без зрозумілого пояснення — бери суму з чека.
...
═══ TOTAL ═══
Пріоритет суми:
1. Спочатку визнач суму з самого чека/рахунку.
2. Потім перевір суму/формулу з підпису прораба.
3. Якщо підпис пояснює логічну різницю ("1462 + 444", доставка, комісія, експлуатація авто, кілька чеків тощо) — використовуй обґрунтовану суму з підпису або порахуй формулу.
4. Якщо підпис вказує додатковий відсоток ("30% від суми", "експлуатація авто 30%") — обов'язково порахуй його від суми чека і додай до total. Приклад: чек 1462 і "30% від суми заправки" => 1462 + 438.6 = 1900.6.
5. Якщо підпис містить суму, яка відрізняється від чека без пояснення або формула використовує неправильну суму чека — віддай перевагу сумі з чека.
6. Якщо суми не видно ніде — null.
...
def extract_receipt_data(image_paths: list[str], caption: str = "", retry: bool = False) -> list:
    ...
    max_tokens = 5000 if not retry else 50000

    reasoning_effort = OCR_RETRY_REASONING_EFFORT if retry else OCR_REASONING_EFFORT
